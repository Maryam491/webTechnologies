...
if (!process.env.JWT_SECRET) {
  console.warn('Warning: JWT_SECRET is not set. API authentication will not work.');
}
...
app.get('/products', async (req, res) => {
  try {
    const { page, limit, skip } = parseProductPagination(req.query, 8);
...
app.post('/cart/add', async (req, res) => {
  if (!req.session.userId) {
    req.flash('error', 'Please sign in to add items to cart.');
    return res.redirect('/auth/login');
  }
  try {
...
app.post('/place-order', isLoggedIn, async (req, res) => {
  try {
    ...
    if (!address || !phone) {
      req.flash('error', 'Address and phone are required');
